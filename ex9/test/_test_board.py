...
def test_initial_works():
    ...
    for row, col in board.cell_list():
        assert board.cell_content((row, col)) is '_'
...
def test_add_car():
    ...
    assert 7 == sum(1 for coord in board.cell_list() if board.cell_content(coord)  != '_')
...
def test_cell_content_works():
    ...
    assert another_board.cell_content(board.target_location()) == '_'
